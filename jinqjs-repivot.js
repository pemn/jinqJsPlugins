  ...
  for(var i = 0; i < rows.length; i++){
    ...
    for(var j = 0; j < rows[i].length; j++){
      ...
      for(var k = 0; k < vdata.length; k++){
        ...
        //table[i][vdata[k] + '_' + revalue] = 1 + angular.isNumber(table[i][vdata[k] + '_' + revalue]) ? 1 : 0; //row[vdata[k]];
        //table[i][vdata[k] + '_' + revalue] = table[i][vdata[k] + '_' + revalue] || 0 + row[vdata[k]];
        table[i][vdata[k] + '_' + revalue] += row[vdata[k]];
        //console.log("k " + k);
        //console.log(row[vdata[k]]);
      }
    }
  }
  //console.log(table);
